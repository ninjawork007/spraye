                              ...
                              // die();
                                if ($estimate_details->status!=3) {
                                  ...
                                  if ($basys_details) {
                                   ...
                                   $line_total = 0; 
                                   $total_tax_amount = 0;
                                   $job_details =  GetOneEstimatAllJobPrice(array('estimate_id'=>$estimate_details->estimate_id));
                                          ...
                                          if ($value2['price_override']!=0) {
                                             $cost =  $value2['price_override'];
                                              ...
                                              if ($priceOverrideData && $priceOverrideData->price_override!=0 ) {
                                                 ...
                                                 $cost =  $value2['job_price'] * $estimate_details->yard_square_feet/1000;
                             ...
                             ?>     
                                  ...
                                  <div class="form-group login-options">
                                      ...
                                      <?php
                                            ...
                                            $line_total = $line_total + $total_tax_amount;
                                            foreach($coupon_details as $coupon) {
                                                $coupon_discount_total = 0;
                                                if ($coupon->coupon_amount_calculation == 0) { // flat
                                                    $coupon_discount_total += $coupon->coupon_amount; 
                                                } else { // perc
                                                    $coupon_discount_total += ($coupon->coupon_amount / 100) * $line_total;
                                                }
                                                $line_total -= $coupon_discount_total;
                                            }
                                           ...
                                           $convenience_fee = $setting_details->convenience_fee*($line_total + $line_tax_amount)/100;


                                       ?>
                                      <span class="text-center no-margin" >Total Amount : <b>$<?= number_format($line_total + $line_tax_amount + $convenience_fee ,2)  ?></b></span>
                                        <?php $line_total -= $total_tax_amount; ?>
                              ...
                              echo '<div class="alert alert-success alert-dismissible" role="alert" data-auto-dismiss="4000"><strong>Estimate </strong> already paid</div>';

                              }
                              
                            } else {
                              echo '<div class="alert alert-danger alert-dismissible" role="alert" data-auto-dismiss="4000"><strong>Estimate </strong> details not found</div>';
                            }
  ...
  submission: (resp) => {
    ...
    switch(resp.status) {
      case 'success':
        ...
        $.ajax({
             url: '<?php echo base_url('welcome/estimatePaymentProcess'); ?>',
             type: 'POST', //$line_total + $line_tax_amount + $convenience_fee
             data: { api_key : '<?= $basys_details->api_key ?>', estimate_id:'<?= $estimate_details->estimate_id ?>',line_total : '<?= number_format($line_total,2,".","")  ?>', total_tax_amount : '<?=  number_format($line_tax_amount,2,".",""); ?>', token : resp.token  },  
             dataType: "JSON",
                ...
                if(response.status == 200){
                   ...
                   swal(
                    ...
                    setInterval(function() {
                      ...
                      window.location.href="<?= base_url('welcome/paymentSuccess/').$setting_details->company_id ?>"; 
