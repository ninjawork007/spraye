...
<html lang="en">
    ...
    foreach ($statement_details as $index => $statement_detail) {
        $property_address_array = explode(',', $statement_detail->property_address);
        $property_address_first = array_shift($property_address_array);
        $property_address_last = implode(',', $property_address_array);
        $property_street_array = Get_Address_From_Google_Maps($statement_detail->property_address);
        $page_break_class = "";
        // die(print_r($statement_detail));
        if ($index > 0) {
        ...
        <div class="container <?php echo $page_break_class ?>">
            ...
            <table width="100%" class="table table-condensed">
                <tr class="first_tr">
                    <td><strong>WORK STATEMENT FOR INVOICE NO: #<?= $statement_detail->invoice_id ?></strong> </td>
                    <td align="right"> <strong> <?= date("m/d/Y", strtotime($statement_detail->statement_date)) ?></strong>
                    </td>
                </tr>
            </table>
            ...
            <table width="100%" class="table table-condensed">
                ...
                <tr>
                    <td align="left">
                        ...
                        <?= $statement_detail->first_name . ' ' . $statement_detail->last_name ?><br>
                        <?php

                        if ($statement_detail->billing_street) {
                            $customer_billing_address = explode(',', $statement_detail->billing_street);
                            ...
                            if ($customer_address_last) {
                                ...
                                echo $statement_detail->billing_city . ', ' . $statement_detail->billing_state . ', ' . $statement_detail->billing_zipcode;
                            }
                        }
                        ?>
                    ...
                    <td align="left">
                        <?php isset($statement_detail->notes) ?  $statement_detail->notes : '';  ?>
                    </td>
                </tr>
            </table>
            ...
            <table width="100%" class="table table-condensed" cellspacing="0">
                ...
                <tbody>
                    ...
                    if (isset($statement_detail->jobs) && is_array($statement_detail->jobs) && !empty($statement_detail->jobs)) {
                        foreach ($statement_detail->jobs as $job) { 
                            // die(print_r($job));
                            ?>
                        ...
                        <td class="text-left" width="30%">
                            <?php
                                    if ($property_street_array && is_array($property_street_array) && !empty($property_street_array)) {
                                        ...
                                        echo $statement_detail->property_city . ', ' . $statement_detail->property_state . ', ' . $statement_detail->property_zip;
                                    }
                                    ?>
                        </td>
                    ...
                    <tr class="border-bottom-blank-td">
                        ...
                        <td class="text-left" width="20%">
                            <?= $statement_detail->job_name ?>
                        </td>
                        <td class="text-left" width="30%">
                            <?= $statement_detail->job_description ?>
                        </td>
                        <td class="text-left" width="10%">
                            <? //if(isset($statement_detail->job_assign_date)){
                                //echo $statement_detail->job_assign_date;
                                //}   
                                ?>
                            <?php if(isset($statement_detail->invoice_date)) {
                                echo date('m/d/Y', strtotime($statement_detail->statement_date));
                            } else if(isset($statement_detail->invoice_created)) {
                                echo date('m/d/Y', strtotime($statement_detail->statement_created));  
                            }?>
                        </td>

                        <td class="text-left" width="10%">
                            <?php echo "$" . number_format($statement_detail->cost, 2);  ?>
                            <?php $total_inv_line_costs = $statement_detail->cost ?>
                        </td>
                    </tr>
                    <?php } ?>
                    ...
                    <tr>
                        <td colspan="2" class="paid_logo cl_50">
                            <?php
                            if ($statement_detail->payment_status == 2) { ?>
                            ...
                            <table class="table table-condensed">
                                ...
                                <tr class="button-tr">
                                    ...
                                    <td class="text-left" colspan="4"><button class="btn btn-success"><a
                                                href="<?= base_url('Welcome/printInvoice/') . $statement_detail->company_id . "/" . $statement_detail->invoice_id ?>"
                                                target="_blank">View Your Invoice</a></button>
                                    </td>
            ...
            <table width="100%" class="main table table-condensed" cellspacing="0">
                ...
                if (isset($statement_detail->jobs) && is_array($statement_detail->jobs) && !empty($statement_detail->jobs)) {
                    ...
                    $jobs[] = array(
                        ...
                        'report' => isset($statement_detail->report_details) ? $statement_detail->report_details : '',
                    );
                }
                    foreach ($jobs as $k => $v) {
                    $i = 0;
                    ...
                    $statement_report_details =  $v['report'];

                    if ($statement_report_details && ($setting_details->is_wind_speed || $setting_details->is_wind_direction || $setting_details->is_temperature || $setting_details->is_applicator_name || $setting_details->is_applicator_number || $setting_details->is_applicator_phone || $setting_details->is_property_address || $setting_details->is_property_size || $setting_details->is_date || $setting_details->is_time)) { ?>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td align="center"
                                                ...
                                                <?= date("m/d/Y", strtotime($statement_report_details->job_completed_date)) ?>
                                                <?php } ?>
                                                <br>
                                                <?php if ($setting_details->is_time == 1) { ?>
                                                <?= date("g:i A", strtotime($statement_report_details->job_completed_time)) ?>
                                                <?php } ?>
                                            </td>
                                            <td align="center"
                                                style="padding-top: 8px !important; padding-bottom: 8px !important;">
                                                <?php if ($setting_details->is_property_address == 1) { ?>
                                                <?= $property_address_first . ', ' . $statement_detail->property_city . ', ' . $statement_detail->property_state . ', ' . $statement_detail->property_zip  ?>
                                                <?php } ?>
                                            </td>
                                            <td align="center"
                                                style="padding-top: 8px !important; padding-bottom: 8px !important;">
                                                <?php if ($setting_details->is_property_size == 1) { ?>
                                                <?= $statement_detail->yard_square_feet  . ' Sq. Ft.' ?>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                    </tbody>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td align="center"
                                                ...
                                                <?= $statement_report_details->user_first_name . ' ' . $statement_report_details->user_last_name ?>
                                                <?php } ?>
                                            </td>
                                            <td align="center"
                                                style="padding-top: 8px !important; padding-bottom: 8px !important;">
                                                <?php if ($setting_details->is_applicator_number == 1) { ?>
                                                <?= $statement_report_details->applicator_number ?>
                                                <?php } ?>
                                            </td>
                                            <td align="center"
                                                style="padding-top: 8px !important; padding-bottom: 8px !important;">
                                                <?php if ($setting_details->is_applicator_phone == 1) { ?>
                                                <?= $statement_report_details->applicator_phone_number ?>
                                                <?php } ?>
                                            </td>
                                            <td align="center"
                                                style="padding-top: 8px !important; padding-bottom: 8px !important;">
                                                <?php if ($setting_details->is_wind_speed == 1) {    ?>
                                                <?= $statement_report_details->wind_speed ?>
                                                <?php if ($setting_details->is_wind_direction == 1) { ?>
                                                <?= $statement_report_details->direction ?>
                                                <?php }
                                                        } ?>
                                            </td>
                                            <td align="center"
                        ...
                        <table width="100%" class="table table-condensed inside-tabel mannual application_tbl"
                            ...
                            <tbody>
                                ...
                                <?php foreach ($product_details as $key => $product_details_value) {
                                                    ...
                                                    $estimated_chemical_used = estimateOfPesticideUsed($product_details_value, $statement_detail->yard_square_feet);
