   ...
   <div class="panel-body">
      ...
      <div id="itemtablediv">
         <div  class="table-responsive table-spraye">
            <table class="table" id="item_quantity_by_location">
               <thead>
                  <tr>
                        <th>Item Name</th>
                        <th>Total Units at <?php echo $location_name; ?></th>
                  </tr>
               </thead>
            </table>
         </div>
...
<script type="text/javascript">
   $(document).ready(function(){
	...
	// console.log("Ajax Call");
	 ...
	 var table =  $('#item_quantity_by_location').DataTable({
		   "processing": true,
		   "serverSide": true,
		   "paging":true,
		    "pageLength":100,
			"order":[[0,'asc']],
		   "ajax":{
		     "url": "<?= base_url('inventory/Frontend/Items/ajaxGetItemQuantityByLocation/')?>",
		     "dataType": "json",
		     "type": "POST",
		     "data":{ '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>', 'location_id': $('#location_id').val()},
           ...
           $("div.toolbar")
              .html('<a href="<?php echo base_url('inventory/Frontend/Items/exportItemLocationCSV?'); ?>' + $('#location_id').val() + '"><button type="button" class="btn btn-success">Export CSV</button></a><a href="" data-toggle="modal" data-target="#modal_choose_loc" id="modal_location_btn" class="modal_trigger_item" ><button type="button" class="btn btn-primary" id="chooselocbtn">View New Location</button></a><a href="" data-toggle="modal" data-target="#modal_choose_subloc" id="modal_sublocation_btn"><button type="button" class="btn btn-primary" id="choosesubbtn" class="modal_trigger_item">View Sub-Location</button></a><a href="<?php echo base_url('inventory/Frontend/Items/overall_item_quantity'); ?>" ><button type="button" class="btn btn-info" id="resetbtn"><i class="icon-reset position-left"></i>Reset</button></a>');
        },
