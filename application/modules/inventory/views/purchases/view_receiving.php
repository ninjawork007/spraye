                            ...
                            <div class="row mt-4">
                                ...
                                <div class="col-md-4 text-break pl-2 pr-2">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="tax" id="tax" class="form-control" value="<?= $purchase_receiving[0]->tax ?>"  />
                                        <div class="invalid-feedback"></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row mt-n3">
                                <div class="col-md-6 text-break pl-2 pr-2 mt-3">
                                    <div class="form-group">
                                        ...
                                        <textarea name="purchase_order_order_notes" id="purchase_order_order_notes" class="form-control" rows="6"><?= $purchase_receiving[0]->notes ?></textarea>
                                    </div>
                                </div>
                                    ...
                                    <table id="summary" class="table stacked">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td width="60" data-summary-field="tax"><?= $purchase_receiving[0]->tax ?>%</td>
                                            </tr>
...
function receivedOrder(){
    var receiving_id = <?= $receiving_id ?>;
    var url = '<?= base_url('inventory/Backend/Purchases/receivedOrder/') ?>'+receiving_id;
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            var purchase_receiving_id = received.purchase_receiving_id;
            var purchase_order_number = received.purchase_order_number;
            var location = received.location_name;
            var subLocation = received.sub_location_name;
            var vendor = received.vendor_name;

            $('span[name=title]').html(purchase_order_number);
            $('#location_name').val(received.location_name);
            $('#location_id').val(received.location_id);
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    response.result.forEach(sublocation => {
                        if(sublocation.sub_location_id == received.sub_location_id){
            ...
            $('#vendor_name').val(received.vendor_name);
            $('#vendor_id').val(received.vendor_id);
            $('input[name=freight]').val();

            $('table#items tbody').html('');

            received.items = JSON.parse(received.items);
            
            Object.values(received.items).forEach((item, i) => {
                
                received_item = item.received_qty;
                total_units += Number(item.quantity);
                received_qty += Number(received_item);
                let unit_price = item.unit_price;
                let receiving = item.receiving_qty;
                let received = received_item;
            ...
            received.total_units = total_units;
            received.receiving_total = received_qty;
            ...
            received['total_received_amount'] = grand_total;
...
function updateTotals() {
    ...
    Object.values(received.items).forEach((item, i) => {
        ...
        received.items[i]['received_qty'] = received_qty;
        received.items[i]['receiving_qty'] = Number(receiving_qty);
    ...
    if(final == total_qty){
        received.purchase_order_status = 3;
		received.is_complete = 1;
    } else {
        received.purchase_order_status = 2;
		received.is_complete = 0;
    }
    received['receiving_total'] = receiving_total;
    ...
    received['subtotal_received'] = subtotal;
    // GETTING THE TOTAL $ AMOUNT OF PURCHASE ORDER
    received['total_received_amount'] = grand_total;
...
function receiveItems() {
    ...
    let data = {
        ...
        tax: $('input[name=tax]').val(),
        subtotal_received: parseFloat(received.subtotal_received).toFixed(2),
        total_received_amount: received.total_received_amount,
        notes: $('textarea[name=purchase_order_order_notes]').val(),
        status: received.purchase_order_status,
	    completed: received.is_complete,
        total_received: received.receiving_total,
        total_units: received.total_units,
        items: []
    }
    
    Object.values(received.items).forEach(item => {
        data.items.push({
            item_id: item.item_id,
            item_number: item.item_number,
            name: item.name,
            receiving_qty: Number(item.receiving_qty),
    ...
    $.ajax({
            ...
            success: function (data){
                ...
                swal(
                    ...
                    location.replace('<?= base_url('inventory/Frontend/Purchases/receiving') ?>');
                    });
...
function receiveAll() {
	...
	Object.values(received.items).forEach((item, i) => {
        ...
        // Update quantity in the original array
        received.items[i]['received_qty'] = received_qty;
        received.items[i]['receiving_qty'] = receiving_qty;
        ...
        total_qty += Number(item.quantity);
        receiving_total += Number(receiving_qty);
        received_total += Number(received_qty);
    ...
    let data = {
        ...
        status: received.purchase_order_status,
	    completed: received.is_complete,
        total_received: received.total_units - received.total_received,
        total_units: received.total_units,
        items: []
    }
