...
class VendorsModel extends CI_Model {
    ...
    public function createNewVendor($post) {
		$query = $this->db->insert(self::VENDORTBL, $post);
    ...
    public function getVendorDataAjaxSearch($where_arr = '', $where_like = '', $limit, $start, $col, $dir, $search, $is_for_count)
    {
        $this->db->select("*");
		...
		$this->db->or_like('internal_name',$search);
		$this->db->or_like('company_name',$search);
        $this->db->or_like('vendor_email_address',$search);
        $this->db->or_like('vendor_phone_number',$search);
        $this->db->or_like('vendor_number',$search);
		$this->db->group_end();
    ...
    public function updateVendorsTbl($vendor_id, $post_data) {
        $this->db->where('vendor_id', $vendor_id);
        return $this->db->update('vendors_tbl', $post_data);
      }
}