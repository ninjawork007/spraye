...
// Selected Programs/Services Box
.selected-program {
    border-color: #00ff2b;
}
...
<div class="content form-pg ">
  ...
  <div class="panel panel-flat">
    <div class="panel-heading">
      <h5 class="panel-title">
        <div class="form-group">
          <a href="<?= base_url('technician/jobDetails/').$selectedProperty[0]->property_id ?>"  id="save" class="btn btn-success" ><i class=" icon-arrow-left7"> </i> Back to Route</a>
          <a href="<?= base_url('technician/Estimates/addServiceEstimate/').$selectedProperty[0]->property_id ?>"  id="save" class="btn btn-success" >New Service Estimate</a>
          <!-- <a href="<?= base_url('technician/Estimates/bulkRenewalProgramsList') ?>"  id="save" class="btn btn-success" >Bulk Renewal</a> -->
          <!-- <a href="<?= base_url('technician/') ?>"  id="save" class="btn btn-success" ><i class=" icon-arrow-left7"> </i> Back to Dashboard</a> -->
        </div>
      </h5>
    </div>
    <br>
    <div class="panel-body">
          ...
          <table id="dataTable_propList" class="table datatable-filter-custom">
            ...
            <tbody>
              ...
              if(!empty($selectedProperty)){
                foreach($selectedProperty as $p){ 
                  ?>
                  <tr>
                    <td>
                      <input type="checkbox" name="property_id" value="<?= $p->property_id ?>" class="row_select"   />
                    </td>
                    <td>
                      <?= $p->property_title ?>
                    </td>
                    ...
                    <td>
                      <input name="price_override" type="checkbox" value="<?= $p->property_id ?>" class="price_override"  />
                    </td>
                    <td>
                      <?= $p->property_zip ?>
                    </td>
      ...
      <form id="submit_form" action="<?= base_url('technician/Estimates/addBulkEstimateData')  ?>" method="post" name="" enctype="multipart/form-data" >
        <div class="row invoice-form">
          <div class="col-md-6">
            <div class="form-group">
              <label class="control-label col-lg-3">Estimate Date</label>
              <div class="col-lg-9">
                <input id="estimate_date" type="text" name="estimate_date" class="form-control pickaalldate" value="<?= date('Y-m-d'); ?>" required>
              </div>
            </div>
          </div>
        ...
        <div class="row invoice-form">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="col-lg-9">
                ...
                <input type="hidden" name="sales_rep" value="<?= $sold_by->id ?>" />
              </div>
            </div>
          </div>
          <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="multi-select-full col-lg-9">
              <select class="multiselect-select-all-filtering" multiple="multiple" name="standalone_job_ids[]" id="job_list">
                <?php
        ...
        <div class="row invoice-form">
          <div class="col-md-6">
            <div class="form-group">
              <label class="control-label col-lg-3">Customer Message<br>(Included in Email)</label>
              <div class="col-lg-9">
                <textarea id="notes" type="text" class="form-control" name="notes" placeholder="Enter Notes" rows="5"></textarea>
              </div>
            </div>       
          </div>
          ...
          <div class="form-group col-lg-4">
            <div class="row">
              ...
              <div class="col-lg-4 text-left">
                ...
                <div class="col-lg-4 text-left">
                <button type="submit" class="btn btn-success" id="submit_estimate_signwell">Submit & Send to SignWell<i class="icon-arrow-right14 position-right"></i></button>                
                </div>
            </div>
          </div>
        </div>
  ...
  if(programsAndServices.length < 1) {
    ...
    allData.forEach((row,i1) => {
      if(selectedPrograms.length>0) {
        ...
        t.program_jobs = [];
        psSorted.forEach((p) => { p.program_jobs.forEach((j) => { t.program_jobs.push(j)})});
...
$(document).ready( function () {
  ...
  // needed to trigger click on page load
  $('#row_select_all').click(); 
...
function getAllServicesByProgram(program_id) {
  $.ajax({
      type: "POST",
      url: "<?= base_url('technician/Estimates/getAllServicesByProgram')  ?>",
