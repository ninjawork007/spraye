                                ...
                                // if ($invoice_details->payment_status!=2) {
                                  if ($basys_details) {
                                    // if ($tax_details) {
                                      ...
                                      //  $convenience_fee = $setting_details->convenience_fee*($invoice_details[0]->cost + $total_tax_amount - $invoice_details[0]->partial_payment)/100;
                                      //  $total_payment_final = $actual_total_cost_miunus_partial + $convenience_fee - $invoice_details[0]->partial_payment;
                                     
                                       ?>
                                      <span class="text-center no-margin" >Total Amount : <b>$<?= number_format($total_payment_final,2)  ?></b></span>
                                     <?php
                                     $string_ids = implode(" ",$invoice_id);
                                    //  echo $string_ids;
...
// console.log(tokenized);
    ...
    switch(resp.status) {
      case 'success':
        ...
        // var invoices = $('input[name]:invoices[]').val();
        console.log(invoices);
        
        $.ajax({
             url: '<?php echo base_url('customers/welcome/paymentProcessPastDue/basys'); ?>',
             type: 'POST',
             data: { 
               api_key : '<?= $basys_details->api_key ?>', 
               'invoice_id': invoices, 
               token : resp.token  },                    
             dataType: "JSON",
