...
class Dashboard_model extends CI_Model{
    ...
    public function getTableRouteDataAjax($where_arr = '', $where_like = '', $limit, $start, $col, $dir, $is_for_count) {
        $this->db->select("technician_job_assign.technician_job_assign_id, technician_job_assign.technician_id, technician_job_assign.route_id, route.route_name, technician_job_assign.job_assign_date, technician_job_assign.customer_id, technician_job_assign.job_id, technician_job_assign.program_id, property_tbl.*");
	...
	public function getCustomerAllServices($where_arr = '') {
        ...
        $this->db->join('property_program_job_invoice', 'property_program_job_invoice.customer_id = customers.customer_id AND property_program_job_invoice.program_id = programs.program_id AND property_program_job_invoice.job_id = jobs.job_id', 'left');
	...
	public function getCustomerUnschedServ($where_arr = '') {
		$this->db->select("jobs.job_id,job_name,customer_property_assign.customer_id,jobs.job_id,`property_tbl`.`property_id`,property_program_assign.program_id,property_title, property_address, programs.program_name");
        $this->db->from('jobs');
		$this->db->join('program_job_assign','program_job_assign.job_id =jobs.job_id','inner');
		$this->db->join('property_program_assign','property_program_assign.program_id = program_job_assign.program_id','inner');
		$this->db->join('property_tbl','property_tbl.property_id = property_program_assign.property_id','inner');
