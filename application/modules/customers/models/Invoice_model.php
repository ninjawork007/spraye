...
class Invoice_model extends CI_Model{   
    ...
    public function getSumInvoive($where_arr) {
        ...
        $this->db->select(' sum( IFNULL(tax_amount, 0 ) ) + cost as cost , partial_payment , sum( IFNULL(tax_amount, 0 ) ) + cost - partial_payment as remaning_amount ,  invoice_tbl.invoice_id ');
    ...
    public function getOneInvoice($where_arr='') {
     ...
     public function getAllInvoiveActive($where_arr='', $whereArrExclude) {

        $this->db->select('invoice_tbl.*,customers.*,property_tbl.*,programs.*,jobs.*,programs.program_price AS programs_program_price, technician_job_assign.is_complete AS tech_job_assign_complete');
    ...
    public function getActiveInvoices($where_arr='', $whereArrExclude, $whereArrExclude2, $orWhere = '') {

        $this->db->select('invoice_tbl.*, CONCAT(customers.first_name, " ", customers.last_name) AS customer_name, property_tbl.property_address, email, programs.program_price AS programs_program_price, technician_job_assign.is_complete AS tech_job_assign_complete, refund_invoice_logs.refund_datetime', false);
        $this->db->from('invoice_tbl');
         $this->db->join('property_tbl','property_tbl.property_id = invoice_tbl.property_id','inner');
        $this->db->join('customers','customers.customer_id = invoice_tbl.customer_id ','inner');
        $this->db->join('programs','programs.program_id = invoice_tbl.program_id','left');
...
//                }
    ...
    public function getUnpaidInvoices($customer_id){
        ...
        $this->db->where(array('customer_id' => $customer_id, 'status !=' => 0, 'payment_status !=' => 2));
