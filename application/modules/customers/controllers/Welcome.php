...
//     }
    ...
    public function paymentProcess($value = '')
    {
        ...
        $all_partial_payments = $this->PartialPaymentModel->getAllPartialPayment(
            array(
        ...
        if ($basys_reasopnse['status'] == 200) {
            ...
            if ($basys_reasopnse['result']->data->response_code == 100) {
                ...
                ##### Create Payment Log #####
                $result = $this->PartialPaymentModel->createOnePartialPayment(
                    array(
                        'invoice_id' => $data['invoice_id'],
        ...
        foreach ($customer_wise_data as $customer_id => $customer_data) {
            ...
            if (is_array($company_email_details)) {
				...
				if(isset($company_email_details['one_day_prior_status']) && $company_email_details['one_day_prior_status'] == 1){
					...
					$company_email_details['one_day_prior'] = str_replace('{PROPERTY_ADDRESS}',constructPropertyAddress((object)($customer_data)),$company_email_details['one_day_prior']);
    ...
    public function dailyPayment($hashstring)
    {
        if ($hashstring != "") {
            ...
            if ($invoice_ids != "") {
                ...
                $invoice_arr = $this->INV->getAllInvoiveWhereIn('invoice_id', $invoice_ids_list);
                foreach ($invoice_arr as $invoice_details) {
                    if ($invoice_details->payment_status != 2) {
                        // array_push($unpaid_invoice_ids, $invoice_details->invoice_id);
                        ...
                        array_push($unpaid_invoice_ids, $invoice_details->invoice_id);
                        $invoice_data_cost = calculateInvoiceCost($invoice_details);
                        $total_tax_amount += $invoice_data_cost->invoice_total_tax;
                        $total_amount_minus_partials += $invoice_data_cost->balance_due;
                        $invoice_cost += $invoice_data_cost->balance_due;
                    }
                }
                if ($invoice_cost > 0) {
                    ...
                    $data['convenience_fee'] = number_format(($data['setting_details']->convenience_fee * ($total_amount_minus_partials) / 100), 2);
                    $data['total_amount_minus_partials'] = $total_amount_minus_partials + $data['convenience_fee'];
    ...
    public function daily_paymentProcess()
    {
        ...
        if (count($invoice_id_list) > 0) {
            $invoice_arr = $this->INV->getAllInvoiveWhereIn('invoice_id', $invoice_id_list);
            $invoice_details = $invoice_arr[0];
            foreach ($invoice_arr as $invoice_data) {
                $invoice_cost_data = calculateInvoiceCost($invoice_data);
                $total_amount_minus_partials += $invoice_cost_data->balance_due;
        ...
        $convenience_fee = number_format(($setting_details->convenience_fee * ($total_amount_minus_partials) / 100), 2);
        $total_amount = $total_amount_minus_partials + $convenience_fee;
        ...
        if ($basys_response['status'] == 200) {
            if ($basys_response['result']->data->response_code == 100) {
                ...
                foreach($invoice_arr as $invoice) {
                    //Create log of partial payments and update invoice
    ...
    public function ccPaymentProcess($value = '')
    {
        ...
        $invoice_cost_data = calculateInvoiceCost($invoice_details);
        $invoice_total_cost = $invoice_cost_data->balance_due;
        $convenience_fee = $setting_details->convenience_fee * ($invoice_total_cost) / 100;
    ...
    public function paymentProcessPastDue($value = '')
    {
        ...
        //v
        $actual_total_cost_miunus_partial = $actual_cost;
        ...
        $data['total_tax_amount'] = $total_tax_amount;
        // die(print_r($convenience_fee));
        $data['total_payment_final'] = $total_amount;
        // die(print_r($total_amount));
        // $dataToLog = array($total_amount);
        // $data = implode(" - ", $dataToLog);
        // $data .= PHP_EOL;
    ...
    public function paymentTotal($invoice_id)
    {
        ...
        if ($invoice_details) {
            ...
            //                 $actual_payments += $amount->paymen_applied;
            //             }
            //         }
            //     }
            // }
    ...
    public function paymentProcessTotal($value = '')
    {
        ...
        $post = array(
            ...
            "tax_amount" => round($total_tax_amount * 100),
            "currency" => "USD",
            "description" => "Invoice for CustomerId: " . $invoice_arr[0]->customer_id,
            "order_id" => $order_id,
            //"ip_address" => $this->getClientIp(),
            "email_receipt" => true,
            "email_address" => $invoice_arr[0]->email,
            "create_vault_record" => true,
            "payment_method" => array(
                "token" => $data['token'],
            ),
            "billing_address" => array(
                "first_name" => $invoice_arr[0]->first_name,
                "last_name" => $invoice_arr[0]->last_name,
                "company" => $invoice_arr[0]->customer_company_name,
                "address_line_1" => $invoice_arr[0]->billing_street,
                "address_line_2" => isset($invoice_arr[0]->billing_street_2) ? $invoice_arr[0]->billing_street_2 : "",
                "city" => isset($invoice_arr[0]->billing_city) ? $invoice_arr[0]->billing_city : "",
                "state" => isset($invoice_arr[0]->billing_state) ? $invoice_arr[0]->billing_state : "",
                "postal_code" => isset($invoice_arr[0]->billing_zipcode) ? $invoice_arr[0]->billing_zipcode : "",
                "email" => $invoice_arr[0]->email,
                "phone" => $invoice_arr[0]->phone,
            ),
        );
        // die(print_r($convenience_fee));
        if ($convenience_fee != 0) {
            ...
            if (strcmp($cc_authorize['result']->respstat, 'A') == 0) {
                ...
                $cap = array(
                    ...
                    'capData' => array(
                        ...
                        'retref' => $cc_authorize['result']->retref,
                        'authcode' => $cc_authorize['result']->authcode,
                        'amount' => $cc_authorize['result']->amount,
                        'invoiceid' => $data['invoice_id'],
                    ),
