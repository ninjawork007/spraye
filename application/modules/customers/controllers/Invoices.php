...
class Invoices extends MY_Controller {
	...
	*/
    ...
    //     {            
    //         $invoices = $this->INV->ajaxActiveInvoices($whereArr,$limit,$start,$order,$dir);
			
    //     }
    //     else {
    //         $search = $this->input->post('search')['value']; 

    //         $invoices =  $this->INV->ajaxActiveInvoicesSearch($whereArr,$limit,$start,$search,$order,$dir);
			
    //         $filteredData = count((array)$invoices);;
    //     }
	// 	$asdf = strval($this->db->last_query());
		
	// 	//remove invoices for incomplete services with method of invoice at completion
	// 	if(!empty($invoices)){
	// 		foreach ($invoices as $key=>$invoice){
	// 			if($invoice->program_price == 2){ 
	// 				//echo "program price = 2";
	// 				//check for completed
	// 				$job_status = $this->Tech->checkIfCompleted(array('invoice_id'=>$invoice->invoice_id));
					
	// 				if($job_status){
	// 					foreach($job_status as $k=>$j){
	// 						if($j['is_complete'] != 1){
	// 							unset($invoices[$key]);
	// 						}
	// 					}
	// 				}else{
	// 					//check for invoice report id.  invoices completed with previous process will all have reports
	// 					if(empty($invoice->report_id)){
	// 						//if no report id in the invoice table, we check the property program job invoice table for report ids
							
	// 						$ppjobinv = $this->PropertyProgramJobInvoiceModel->getAllRows(array('invoice_id'=>$invoice->invoice_id));
	// 						if($ppjobinv){
	// 							foreach($ppjobinv as $k2=>$j2){
	// 								if(empty($j2['report_id'])){
	// 									unset($invoices[$key]);
	// 								}
	// 							}
	// 					   }
    ...
    //             $nestedData['customer_id'] = $invoice->customer_name;
	// 			$nestedData['email'] = $invoice->email;
	// 			// $nestedData['email'] = $asdf;
				
	// 			$total_tax_amount = getAllSalesTaxSumByInvoice($invoice->invoice_id)->total_tax_amount;
	// 			$nestedData['cost'] = '$ '.number_format($invoice->cost+$total_tax_amount,2);
				
	// 			$due = $invoice->cost+$total_tax_amount-$invoice->partial_payment;
    ...
    //                        case 0:
    //                          $status .= '<span  class="label label-warning myspan">Unsent</span>';
    //                          $bg= 'bg-warning';
    //                          break;
    //                        case 1:
    //                          $status .= '<span  class="label label-danger myspan">Sent</span>';
    //                          $bg= 'bg-danger';
    //                          break;
                           
    //                        case 2:
    //                          $status .= '<span  class="label label-success myspan">Paid</span>';
    //                         $bg= 'bg-success';
    //                         break;

    //                        case 3:
    //                          $status .= '<span  class="label label-till myspan">Partial</span>';
    //                           $bg= 'bg-till';
    //                         break;
                            
    //              }
	// 			 $status .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changestatus"  invoice_id="'.$invoice->invoice_id.'" value="0" ><a href="#"><span class="status-mark bg-warning position-left"></span> Unsent</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="1" ><a href="#"><span class="status-mark bg-danger position-left"></span> Sent</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Paid</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="3"  over_all_total="'.floatval($invoice->cost+$total_tax_amount).'" partial_payment="'.floatval($invoice->partial_payment).'"><a href="#"><span class="status-mark bg-till position-left" ></span> Partial</a></li></ul></div>';
	...
	// 				$actions = '<span class="pr-5"><a class="email button-next" id="'.$invoice->invoice_id.'" customer_id="'.$invoice->customer_id.'"><i class="icon-envelop3 position-center" style="color: #9a9797;"></i></a></span><span class="pr-5"><a class=""><i class="icon-info22 position-center" style="color: #9a9797;" onclick="productDetailsGet('.$invoice->job_id.')"></i></a></span><span><a href="'.base_url('admin/Invoices/pdfInvoice/').$invoice->invoice_id.'" target="_blank" class=" button-next"><i class="icon-printer2 position-center" style="color: #9a9797;"></i></a></span>';
	// 			}else{
	// 				$actions = '<span class="pr-5"><a class="email button-next" id="'.$invoice->invoice_id.'" customer_id="'.$invoice->customer_id.'"><i class="icon-envelop3 position-center" style="color: #9a9797;"></i></a></span><span class="pr-5"><a class=""><i class="icon-info22 position-center" style="color: #9a9797;" onclick="productDetailsGetByInvoice('.$invoice->invoice_id.')"></i></a></span><span><a href="'.base_url('admin/Invoices/pdfInvoice/').$invoice->invoice_id.'" target="_blank" class=" button-next"><i class="icon-printer2 position-center" style="color: #9a9797;"></i></a></span>';
	// 			}
	...
	public function ajaxGetActive() {
		...
		if(!empty($invoices)){
			...
			foreach ($invoices as $invoice){	
				...
				$nestedData['email'] = $invoice->email;
                // $nestedData['customer_id'] = $invoice->programs_program_price;
				// $nestedData['email'] = $invoice->tech_job_assign_complete;
				// $nestedData['email'] = strval($this->db->last_query());
				// $nestedData['email'] = json_encode($whereArr);
				...
				switch ($invoice->status) {
					...
					break;
					case 2:
						$status .= '<span class="label label-success myspan">Opened</span>';
						$bg= 'bg-success';
						$sent_date = isset($invoice->sent_date) ? date('Y-m-d', strtotime($invoice->sent_date)) : "";
					...
					case 3: //the old status == 3 was for partial payments
						$status .= '<span class="label label-till myspan">Opened </span>';
						$bg= 'bg-till';
						$sent_date = isset($invoice->sent_date) ? date('Y-m-d', strtotime($invoice->sent_date)) : "";
						$open_date = isset($invoice->opened_date) ? date('Y-m-d', strtotime($invoice->opened_date)) : "";
					break;
				...
				// $status .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changestatus"  invoice_id="'.$invoice->invoice_id.'" value="0" ><a href="#"><span class="status-mark bg-warning position-left"></span> Unsent</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="1" ><a href="#"><span class="status-mark bg-danger position-left"></span> Sent</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Paid</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="3"  over_all_total="'.floatval($invoice->cost+$total_tax_amount).'" partial_payment="'.floatval($invoice->partial_payment).'"><a href="#"><span class="status-mark bg-till position-left" ></span> Partial</a></li></ul></div>';
				$status .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changestatus"  invoice_id="'.$invoice->invoice_id.'" value="0" ><a href="#"><span class="status-mark bg-warning position-left"></span> Unsent</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="1" ><a href="#"><span class="status-mark bg-danger position-left"></span> Sent</a></li><li class="changestatus" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Opened</a></li></ul></div>';
				...
				switch ($invoice->payment_status) {
					case 0:
						if ($partial_amount_paid > 0){
                            if ($due <= 0){
                                $payStatus .= '<span class="label label-success myspan">Paid</span>';
						        $bg= 'bg-success';
                                ...
                                $payStatus .= '<span class="label label-till myspan">Partial</span>';
						        $bg= 'bg-till';
						        $payment_date = isset($invoice->payment_created) ? ($invoice->payment_created == '0000-00-00 00:00:00' ? ($invoice->last_modify != '0000-00-00 00:00:00' ? date('Y-m-d', strtotime($invoice->last_modify)) : '' ) : date('Y-m-d', strtotime($invoice->payment_created)))  : "";
                                break;
                            }
                            ...
                            $payStatus .= '<span class="label label-warning myspan">Unpaid</span>';
						    $bg= 'bg-warning';
                        }
					...
					break;
					case 3:
						$payStatus .= '<span  class="label label-danger myspan">Past Due</span>';
						$bg= 'bg-danger';
					break;
					case 4:
						$payStatus .= '<span  class="label label-refunded myspan">Refunded</span>';
						$bg= 'bg-refunded';
						$payment_date = isset($invoice->payment_created) ? ($invoice->payment_created == '0000-00-00 00:00:00' ? ($invoice->last_modify != '0000-00-00 00:00:00' ? date('Y-m-d', strtotime($invoice->last_modify)) : '' ) : date('Y-m-d', strtotime($invoice->payment_created)))  : "";
				...
				if ($due < $invoice_total_cost && $due > 0){
                    $payStatus .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="1"  over_all_total="'.floatval($invoice->cost+$total_tax_amount).'" partial_payment="'.floatval($invoice->partial_payment).'"><a href="#"><span class="status-mark bg-till position-left" ></span> Partial</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Paid</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="3"><a href="#"><span class="status-mark bg-danger position-left"></span> Past Due</a></li></ul></div>';
					// Paid Status
                } else if ($due < $invoice_total_cost && $due <= 0 && $invoice->payment_status != 4) {
                    $payStatus .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="4"><a href="#"><span class="status-mark bg-refunded position-left"></span> Refund </a></li></ul></div>';
                    // $payStatus .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Paid</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="4"><a href="#"><span class="status-mark bg-refunded position-left"></span> Refund </a></li></ul></div>';
					// Past Due Status
                } else if ($invoice->payment_status == 3) {
                    $payStatus .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="1"  over_all_total="'.floatval($invoice->cost+$total_tax_amount).'" partial_payment="'.floatval($invoice->partial_payment).'"><a href="#"><span class="status-mark bg-till position-left" ></span> Partial</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Paid</a></li></ul></div>';
					// Refund Status
                } else if ($invoice->payment_status == 4) {
                    $payStatus .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li  total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value=""><a href="#"><span class="status-mark bg-refunded position-left"></span> Refunded </a></li></ul></div>';
                   
                } else {
                    $payStatus .= '<div class="btn-group"><a href="#" class="label '.$bg.' dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a><ul class="dropdown-menu dropdown-menu-right" ><li class="changepayment"  total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="0" ><a href="#"><span class="status-mark bg-warning position-left"></span> Unpaid</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="1"  over_all_total="'.floatval($invoice->cost+$total_tax_amount).'" partial_payment="'.floatval($invoice->partial_payment).'"><a href="#"><span class="status-mark bg-till position-left" ></span> Partial</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="2" ><a href="#"><span class="status-mark bg-success position-left"></span> Paid</a></li><li class="changepayment" total_due="'.$due.'" invoice_id="'.$invoice->invoice_id.'" value="3"><a href="#"><span class="status-mark bg-danger position-left"></span> Past Due</a></li></ul></div>';
                }
...
//$this->form_validation->set_rules('customer_email', 'customer_email', 'required');
	...
	public function editInvoice($invoice_id) {
		...
		$data['job_details'] = $this->JobModel->getAllJob(array('company_id' =>$this->session->userdata['company_id']));
	...
	//die(print_r($data));
  	...
  	public function pdfInvoicescheduled($invoice_id) {
		...
		$this->load->view('customers/invoice/pdf_invoice_scheduled',$data);
...
//        die(print_r($html));
    ...
    public function pendingJobInvoicescheduled($technician_job_assign_ids){
		...
		if(count($data['invoice_details']) > 0 ) {
			$this->load->view('customers/invoice/multiple_pdf_invoice_print_scheduled',$data);
			$html = $this->output->get_output();
			// Load pdf library
			$this->load->library('pdf');
			// Load HTML content
	...
	//die(print_r($data));
      if(count($data['invoice_details']) > 0 ) {
        $this->load->view('customers/invoice/multiple_pdf_invoice_print',$data);
        $html = $this->output->get_output();
        // Load pdf library
        $this->load->library('pdf');
        // Load HTML content
   ...
   /*  public function printInvoice($invoice_ids) {
			...
			// die(print_r($data['company_details']));
			$where_arr = array(
				'company_id' => $this->session->userdata['company_id'],
				'status' => 1
			);
...
//        die(print_r($html));
   ...
   public function getOpenInvoiceByCustomer($customer_id) {
		...
		// die(print_r($data));
        $this->load->view('customers/invoice/customer_all_pdf_invoice',$data);
   ...
   public function bulkChangeStatus($value=''){
    ...
    public function sendPdfMail() {
        ...
        $body = $this->load->view('customers/invoice/email_pdf',$data,true);
...
#### not same as refund. Reset payments to what it was before, like this payment never happened.
	...
	public function printTechWorksheet($technician_job_assign_ids){
		...
		foreach ($tech_ids as $key => $technician_job_assign_id) {
			...
			array_push($data['assigned_job_details'], $this->Tech->getOneTechJobAssign(array('technician_job_assign_id' => $technician_job_assign_id)));
		}
		...
		if(!empty($data['job_assign_details'])){
			...
			$data['technician_route_details'] = array();
				...
				foreach ($data['job_assign_details'] as $key => $value) {
					...
					$technician_route_details = array(
						...
						'wind_speed' => $data['assigned_job_details'][$key]->max_wind_speed,
						'temp' => $data['assigned_job_details'][$key]->temperature_information,
						'tech_job_assign_id' => $data['assigned_job_details'][$key]->technician_job_assign_id,
						'product_id' => $data['assigned_job_details'][$key]->product_id,
						'epa_reg_nunber' => $data['assigned_job_details'][$key]->epa_reg_nunber,
						'weed_pest_prevented' => $data['assigned_job_details'][$key]->weed_pest_prevented,
						're_entry_time' => $data['assigned_job_details'][$key]->re_entry_time,
						'route_name' => $data['assigned_job_details'][$key]->route_name,
						'user_first_name' => $value[0]['user_first_name'],
						'user_last_name' => $value[0]['user_last_name'],
						'amount_used'=> $used_mixture,
			...
			if (count($data['route_results']) > 0) {
				$this->load->view('customers/invoice/multiple_pdf_technician_print', $data);
				$html = $this->output->get_output();
    ...
    public function downloadInvoiceCSV(){
       ...
       // WHERE:
        ...
        if(empty($this->input->post('search')['value'])) {       
            ...
            $search = $this->input->post('search')['value']; 
			$invoices =  $this->INV->ajaxActiveInvoicesSearchTech($whereArr,$limit,$start,$search,$order,$dir, $whereArrExclude, $whereArrExclude2, $orWhere, true);
		...
		if(!empty($invoices)){
                    ...
                    //set column headers
                    $fields = array('Invoice Id','Customer Name','Customer Email','Invoice Amount','Balance Due','Sent Status','Payment Status','Invoice Date','Sent Date','Opened Date','Payment Date',);
                    ...
                    foreach ($invoices as $invoice) {
                        ...
                        switch ($invoice->payment_status) {
                            ...
                            case 4:
                                ...
                                $lineData = array($invoice->invoice_id, $invoice->customer_name, $invoice->email, '$'.number_format($invoice_total_cost,2), $balance_due, $sent_status, $payStatus, date('m-d-Y', strtotime($invoice->invoice_date)), $sent_date, $open_date, $payment_date );
