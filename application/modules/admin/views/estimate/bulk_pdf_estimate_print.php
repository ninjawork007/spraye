...
?>
<?php foreach ($estimate_details as $k => $estimate): ?>
<div class="container" >
    ...
    <table width="100%" class="table table-condensed">
        <tr class="first_tr">
            <td><strong>ESTIMATE NO: #<?=$estimate->estimate_id?></strong></td>
            <td align="right"> <strong><?=Date("m/d/Y", strtotime($estimate->estimate_date))?></strong>
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td align="left" width="40%">
                ...
                <table width="100%">
                    ...
                    <?php
                    $customer_billing_address = explode(',', $customer_details[$k]->billing_street);
                    $customer_address_first = array_shift($customer_billing_address);
                    ?>

                    <tr>
                        <td><?=$customer_details[$k]->first_name . ' ' . $customer_details[$k]->last_name?></td>
                    </tr>
                    <tr>
                        <td><?php echo $customer_address_first ?></td>
                    </tr>
                    <tr>
                        <td><?php echo $customer_details[$k]->billing_city . ', ' . $customer_details[$k]->billing_state . ', ' .
                                $customer_details[$k]->billing_zipcode; ?></td>
                    </tr>
                </table>
            ...
            <td align="left" width="40%">
                <?=(isset($estimate->notes))?str_replace(array("\r\n", "\r", "\n"), "<br />", $estimate->notes):''?>
            </td>
    ...
    <table width="100%" class="table table-condensed main_table" cellspacing="0">
        ...
        <tr style="background-color:<?=$setting_details->invoice_color?>!important;color: #fff;">
            ...
            <td class="text-right" style="text-align: right; padding-right: 50px;"><strong>PRICE</strong>
            </td>
        ...
        $sales_tax_details = getAllSalesTaxByProperty($estimate->property_id);

        if ($job_details[$k]) {

            foreach ($job_details[$k] as $key => $value) {
                ...
                if ($value['price_override'] != '' && $value['price_override'] != 0 && $value['is_price_override_set'] == 1) {
                    ...
                    $priceOverrideData = getOnePriceOverrideProgramProperty(array('property_id' => $estimate->property_id, 'program_id' => $estimate->program_id));
                    ...
                    if ($priceOverrideData && $priceOverrideData->price_override != 0 && $priceOverrideData->is_price_override_set == 1) {
                        ...
                        $lawn_sqf = $estimate->yard_square_feet;
                        $job_price = $value['job_price'];

                        //get property difficulty level
                        if (isset($estimate->difficulty_level) && $estimate->difficulty_level == 2) {
                            $difficulty_multiplier = $setting_details->dlmult_2;
                        } elseif (isset($estimate->difficulty_level) && $estimate->difficulty_level == 3) {
                ...
                <tr>
                    <td class="text-left">
                        <?php echo $estimate->property_address ? $estimate->property_address : ''; ?>
    ...
    <table width="100%" class="main_table none_border" style="border: 1px solid #ddd;">
        ...
        <?php if (isset($coupon_estimate[$k]) && !empty($coupon_estimate[$k])) {
            ...
            <?php foreach ($coupon_estimate[$k] as $coupon_details) {
