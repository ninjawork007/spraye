...
<body>
  <?php
	...
	if(isset($invoice_details->is_group_billing) && $invoice_details->is_group_billing == 1){
	?>
	<div class="container" id="group_billing_container">
		...
		<table width="100%" class="table table-condensed" style="margin-bottom: 0px !important;">
			<tr class="first_tr" style="padding:5px!important;">
				<td><strong>WORK ORDER NO : #<?= $invoice_details->invoice_id ?></strong></td>
		...
		<table width="100%" class="table table-condensed">
			...
			<tr>
				<td align="left">
					<?= $invoice_details->group_billing_details['first_name'].' '.$invoice_details->group_billing_details['last_name'] ?><br>
		...
		<table width="100%" class="table table-condensed" cellspacing="0" id="main_table">
			...
			<tbody>
				<?php if(isset($invoice_details->jobs) && is_array($invoice_details->jobs) && !empty($invoice_details->jobs)){
					foreach($invoice_details->jobs as $job){ ?>
						...
						<tr class="border-bottom-blank-td">
							...
							<td class="text-left" width="20%"><?= $invoice_details->job_name ?></td>
							<td class="text-left" width="30%"><?= $invoice_details->job_description ?></td>
							<td class="text-left" width="10%">
								<?php if(isset($invoice_details->job_assign_date)){
									echo date('m/d/Y', strtotime($invoice_details->job_assign_date));
								} elseif(isset($invoice_details->job_completed)) {
									echo date('m/d/Y', strtotime($invoice_details->job_completed)); 
								} elseif(isset($invoice_details->invoice_created)) {
									 echo date('m/d/Y', strtotime($invoice_details->invoice_created)); 
								} elseif(isset($invoice_details->invoice_date)) {
									echo date('m/d/Y', strtotime($invoice_details->invoice_date)); 
								} ?>
							</td>
						</tr>
					<?php } ?>
                    ...
                    <table width="100%" class="table table-condensed" style="margin: -10px 0 20px 0; font-size: 12px;"
                        ...
                        <tr class="first_tr">
                            <td align="left" width="50%" style="padding: 5px 20px 5px 5px!important;">
                                ...
                                <p style="float: right;margin:0px;">
                                    <?= $invoice_details->first_name.' '.$invoice_details->last_name ?></p>
                            </td>
                        ...
                        <tr>
                            <td align="left" width="50%" style="padding-right: 20px;">
                                <b>Invoice Date</b>
                                <p style="float:right; margin:0px;">
                                <?php if(isset($invoice_details->invoice_date)) {
                                echo date('m/d/Y', strtotime($invoice_details->invoice_date));
                                } else if(isset($invoice_details->job_assign_date)){
                                echo date('m/d/Y', strtotime($invoice_details->job_assign_date));
                    ...
                    <?php 
                        ...
                        <tr>
                            <td colspan="2" class="paid_logo cl_50">
                                <?php if($invoice_details->payment_status==2)  { ?>
                            ...
                            <td colspan="2" class="cl_50">
                                ...
                                <table class="table table-condensed" >
                                    ...
                                    <?php if ($invoice_details->all_sales_tax && $invoice_details->payment_status==2 ) { ?>
                                    ...
                                    <tr class="button-tr">
                                        ...
                                        <td class="text-left" colspan="4"><button class="btn btn-success"><a
                                            href="<?= $setting_details->pay_now_btn_link  ?>" target="_blank">Pay Now</a></button>
                                        </td>
