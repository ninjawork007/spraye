...
class Estimate_model extends CI_Model{
   ...
   public function CreateOneEstimate($post) {
    ...
    public function CreateEstimateJobs($job_list=0, $estimate_id=0) {
        //delete any previous data for same estimate id
        $this->db->delete(self::EST_JBS, ['estimate_id' => $estimate_id]);
        $job_list = json_decode(json_encode($job_list), true);
        if(is_array($job_list) && !empty($job_list) && $estimate_id){
            foreach ((array) $job_list as $value) {
                $query = $this->db->insert(self::EST_JBS, ['estimate_id' => $estimate_id, 'job_id' => $value['job_id']]);
            }
            return true; 
        } else {
            return false;
    ...
    public function getEstimatePrograms($where_arr = '') {           
        $this->db->select('estimate_programs.program_id');        
        $this->db->from(self::EST);        
        $this->db->join('estimate_programs','t_estimate.estimate_id = estimate_programs.estimate_id','inner');
        if (is_array($where_arr)) {
          $this->db->where($where_arr);
        }  
        $result = $this->db->get();
        ...
        foreach ($data as $key => $value) {
           $res[] = $value->program_id;
        }
        return $res;
    }
    public function getEstimateServices($where_arr = '') {           
        $this->db->select('estimate_jobs.job_id');        
        $this->db->from(self::EST);      
        $this->db->join('estimate_jobs','t_estimate.estimate_id = estimate_jobs.estimate_id','inner');
        if (is_array($where_arr)) {
          $this->db->where($where_arr);
        }  
        $result = $this->db->get();
    ...
    public function getAllEstimatePriceOveridewJob($where_arr = '') {
        $this->db->select('t_estimate_price_override.*, jobs.job_price, jobs.job_name, jobs.job_price_per, jobs.base_fee_override, jobs.min_fee_override');
        $this->db->from("t_estimate_price_override");
  ...
  public function getEstimatePropertiesByProgram($program_id = 0){
      $this->db->select('property_tbl.* ,estimate_programs.program_id');
      $this->db->from(self::EST_PRGMS);
      $this->db->join('t_estimate','estimate_programs.estimate_id = t_estimate.estimate_id','inner');
      $this->db->join('property_tbl','property_tbl.property_id = t_estimate.property_id','inner');
      
      $this->db->where(['estimate_programs.program_id' => $program_id]);
...
public function getTableDataAjaxSearchEstimateProp($where_arr = '', $where_like = '', $limit, $start, $col, $dir, $search, $is_for_count, $where_in = '') {
    ...
    public function getAllProgramsByPricing($where_arr) {
        $this->db->select("*");
        $this->db->from('programs');
        $this->db->where('company_id',$where_arr["company_id"]);
        $this->db->where('program_price',$where_arr["program_price"]);
        $this->db->where('ad_hoc', 0);
        $this->db->where('program_active', 1);
        $result = $this->db->get();
        $data = $result->result();
        return $data;
    }
    
    public function deleteEstimateProgram($wherearr) {

        if (is_array($wherearr)) {
            $this->db->where($wherearr);
        }
        
        $this->db->delete(self::EST_PRGMS);
