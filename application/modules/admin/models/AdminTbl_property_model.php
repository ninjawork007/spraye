...
//        die(var_dump($time_elapsed_secs));
	...
	public function getCancelledPropertyByDateRange($where,$from='',$to=''){
        $this->db->select('property_tbl.*, customers.customer_id, customers.created_at as start_date, customers.first_name, customers.email, customers.last_name, customers.work_phone, users.user_first_name, users.user_last_name, t_estimate.sales_rep');
        $this->db->from('property_tbl');
