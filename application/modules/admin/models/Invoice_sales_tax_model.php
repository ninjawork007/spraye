...
class Invoice_sales_tax_model extends CI_Model{
    ...
    public function updateInvoiceSalesTax($where_arr, $post){
        $this->db->where($where_arr);
        $this->db->update(self::INTX, $post);
        $this->db->where($where_arr);

        $result = $this->db->get(self::INTX);

        $data = $result->row('sales_tax_id');
...
EOD;
    ...
    public function matchInvTechByInvId($where_arr = '') {
        ...
        $this->db->join('technician_job_assign', 'invoice_tbl.invoice_id = technician_job_assign.invoice_id', 'left');
    ...
    public function matchInvTechByAllFour($where_arr = '') {
        ...
        $this->db->from('invoice_tbl');
        $this->db->join('technician_job_assign', 'technician_job_assign.customer_id = invoice_tbl.customer_id AND technician_job_assign.program_id = invoice_tbl.program_id AND technician_job_assign.property_id = invoice_tbl.property_id AND invoice_tbl.job_id = technician_job_assign.job_id', 'left');
    ...
    public function matchInvTechByTbl($where_arr = '') {
        ...
        $this->db->join('technician_job_assign', 'technician_job_assign.customer_id = invoice_tbl.customer_id AND technician_job_assign.program_id = invoice_tbl.program_id AND technician_job_assign.property_id = invoice_tbl.property_id AND program_job_assign.job_id = technician_job_assign.job_id', 'left');
